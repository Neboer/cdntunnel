...
def cdntunnel(client_socket: socket, server_socket: socket):
    ...
    while True:  # 主循环
        ...
        readable_socket, _, _ = select([client_socket, server_socket], [], [])
        if client_socket in readable_socket:  # 客户端发来连接请求，看来好时代要到来力！
            client_data = CintroMessage.receive_from_socket(client_socket)
        ...
        if server_socket in readable_socket:
            ...
            server_data = server_socket.recv(1000000)  # 从服务端收最大1M的数据，这个地方存疑：
