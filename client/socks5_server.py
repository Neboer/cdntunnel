...
def request_client(client_socket):
    """ Client request details """
    # +----+-----+-------+------+----------+----------+
    # |VER | CMD |  RSV  | ATYP | DST.ADDR | DST.PORT |
    # +----+-----+-------+------+----------+----------+
    # -0    1     2       3      4          x
    # x = 5 + int(req[4]) - len(req) domain name
    # x = len(req) - 2 ipv4
    # 我们只需要取ATYP DST.ADDR DST.PORT就可以。
    # 因此，我们直接把[3:]全部发送，就大丈夫啦~kira☆
    try:
        s5_request = client_socket.recv(BUFSIZE)
...
def request(client_socket):
    """
        ...
        returns a reply
    """
    dst = request_client(client_socket)
    # Server Reply
    # +----+-----+-------+------+----------+----------+
    ...
    # start proxy
    if rep == b'\x00':
        proxy_loop(client_socket, socket_dst)
    if client_socket != 0:
        client_socket.close()
    if socket_dst != 0:
        socket_dst.close()
...
def subnegotiation_client(client_socket):
    """
    ...
    # +----+----------+----------+
    try:
        identification_packet = client_socket.recv(BUFSIZE)
    except socket.error:
        error()
...
def subnegotiation(client_socket):
    """
        The client connects to the server, and sends a version
        identifier/method selection message
    ...
    """
    method = subnegotiation_client(client_socket)
...
def main():
    ...
    while not EXIT.get_status():
        if activeCount() > MAX_THREADS:
            ...
            continue
        try:
            client_socket, _ = new_socket.accept()
            ...
            error()
            sys.exit(0)
        recv_thread = Thread(target=connection, args=(client_socket, ))
        recv_thread.start()
    new_socket.close()
